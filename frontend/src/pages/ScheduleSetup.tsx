...
import { Clock, CalendarDays, Sun, Sunrise, Sunset, Moon, Flame, ArrowRight, ArrowLeft } from "lucide-react";
...
const STUDY_TIMES = [
  { id: "morning", label: "Morning", time: "6AM - 12PM", icon: Sunrise, color: "text-amber-400" },
  { id: "afternoon", label: "Afternoon", time: "12PM - 5PM", icon: Sun, color: "text-orange-400" },
  { id: "evening", label: "Evening", time: "5PM - 9PM", icon: Sunset, color: "text-rose-400" },
  { id: "night", label: "Night", time: "9PM - 2AM", icon: Moon, color: "text-indigo-400" },
];
...
export default function ScheduleSetup() {
  ...
  const [selectedTime, setSelectedTime] = useState<string | null>(schedule?.preferredTime ?? null);
  const [selectedHours, setSelectedHours] = useState<number | null>(schedule?.weeklyCommitment ?? null);

  const canContinue = selectedDays.length > 0 && Boolean(selectedTime) && Boolean(selectedHours);
      ...
      <div className="w-full max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-semibold text-white">Which days will you study?</h2>
            </div>
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-semibold text-white">When are you most focused?</h2>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {STUDY_TIMES.map((time) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isSelected ? 'bg-purple-500/20' : 'bg-white/5'}`}>
                      <Icon className={`w-5 h-5 ${isSelected ? time.color : 'text-gray-500'}`} />
                    </div>
                    <div>
                      <div className={`font-medium ${isSelected ? 'text-white' : 'text-gray-300'}`}>{time.label}</div>
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-semibold text-white">Weekly Commitment</h2>
            </div>
            <div className="grid grid-cols-4 gap-3">
              {HOURS.map((hours) => {
                ...
                return (
                  ...
                  >
                    <span className={`text-xl font-bold ${isSelected ? 'text-orange-400' : 'text-gray-300'}`}>
                      {hours}{hours === 20 ? '+' : ''}
                    </span>
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-24 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl shadow-black/50">
            <h3 className="text-lg font-semibold text-white mb-6">Your Commitment</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 text-sm text-blue-200 leading-relaxed">
                {selectedDays.length > 0 && selectedHours ? (
                  <span>
                    That's about <strong className="text-blue-400">{Math.round((selectedHours / selectedDays.length) * 10) / 10} hours</strong> per study day. 
                    {selectedHours >= 15 ? ' An aggressive pace!' : ' A steady, sustainable pace.'}
                  </span>
                ) : (
